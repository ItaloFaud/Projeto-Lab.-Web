...
class ProdutoDAO{
	...
	function Deletar(Produto $p){
		$pdo = new Conexao;
		$con = $pdo->Conectar();
		$Deleta = $con->prepare("DELETE FROM produto WHERE id = ?");
	...
	function ConsultaUnica(Produto $c){
		$pdo = new Conexao;
		$con = $pdo->Conectar();
	...
	function ConsultaCat($c){
		$pdo = new Conexao;
		$con = $pdo->Conectar();

		$Consulta=$con->prepare("SELECT * FROM produto WHERE idcategoria = ?");
		$Consulta->bindValue(1,$c,PDO::PARAM_INT);
		$Consulta->execute();
